...
<?php
	echo elgg_view('publication/embed/tabs',array('tab' => 'media', 'internalname' => $vars['internalname']));
?>
	<div id='mediaEmbed'>
<?php
	echo elgg_view('embed/pagination',array(
												'offset' => $vars['offset'],
												'baseurl' => $vars['url'] . 'pg/publications/embed/media?internalname=' . $vars['internalname'] . "&amp;simpletype=" . $vars['simpletype'],
												'limit' => $vars['limit'],
												'count' => $vars['count']
											));
	...
	if (is_array($entities) && !empty($entities)) {
		...
		echo "<p class=\"embedInstructions\">" . elgg_echo('publication:attachment:instruction') . "</p>";
